...
let bgm; // ตัวแปรสำหรับเก็บเพลงพื้นหลัง
...
function create() {
    ...
    // ==========================================
    bgm = this.sound.add('bgm', { volume: 1.0, loop: true });
    bgm.play();
    ...
    wallsLayer = map.createLayer('wall', tileset, 0, 0).setDepth(-9);
    ...
    // 💬 4. UI และปุ่มกด
    // ==========================================
    alertText = this.add.text(600, 337, '', {
    ...
    if (nearbyTarget) {
        if (nearbyTarget.getData('kind') === 'fire') {
            ...
            if (Phaser.Input.Keyboard.JustDown(keyE) && canOpenDoor && !exitDoorOpened) {
                ...
                // ✅ แสดงข้อความชนะ
                dialogText.setText('คุณชนะแล้ว! หลบหนีสำเร็จ');
                dialogText.setPosition(player.x, player.y - 60);
                dialogText.setVisible(true);
            }
            ...
            interactText.setText(npcDialogueState === 'idle' ? 'กด E เพื่อคุย' : 'กด Eต่อ / Q เพื่อยกเลิก');
            interactText.setVisible(true);
