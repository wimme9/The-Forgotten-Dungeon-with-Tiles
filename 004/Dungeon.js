...
// === ตัวแปรใหม่: ระบบเสียง (Sound FX & BGM) ===
let bgMusic;
let correctSound;
let winSound;
...
function preload() {
    ...
    // === 🔊 โหลดไฟล์เสียงตามรูปภาพของคุณ ===
    // (หากไฟล์จริงเป็น .wav ให้เปลี่ยนนามสกุลไฟล์ตรงนี้นะครับ)
...
function update() {
    ...
    if (isCutsceneActive) {
        ...
        if (cutsceneStep === 1) {
            ...
            if (distance > 5) {
                ...
                hideDialogue();
                showVictoryScreen(); // จะเรียกเล่นเสียงชนะตรงนี้
    ...
    this.physics.overlap(player, torches, () => { isTouchingSomething = true; });
...
// === 🔊 ฟังก์ชันเปิดหน้าจอชนะ และเล่นเสียงชนะ ===
function showVictoryScreen() {
    // หยุด BGM เดิม และเล่นเสียงชนะ
