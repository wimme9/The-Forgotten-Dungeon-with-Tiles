...
function create() {
    ...
    // ── Tilemap: floor (visual) + wall (collision) ──
    ...
    // ── Invisible collision walls built directly from the "wall" layer's tile data ──
    // This guarantees the collision boxes line up exactly with the map, regardless of
    // whether Tiled's built-in tile collision (setCollisionByExclusion) behaves as expected.
    // Set DEBUG_SHOW_WALLS to true to visualize the generated wall boxes in red.
    const DEBUG_SHOW_WALLS = false; // set true to see the red outlines
...
wallSeg(984, 256, 16,  512);  // right, above exit gap (y 0-512)
wallSeg(984, 688, 16,  96);   // right, below exit gap (y 640-736)
...
console.log('Wall segments created:', walls.getChildren().length);
